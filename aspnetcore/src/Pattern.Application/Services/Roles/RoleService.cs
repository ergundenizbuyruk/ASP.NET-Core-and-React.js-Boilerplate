...
namespace Pattern.Application.Services.Roles
{
    public class RoleService : ApplicationService, IRoleService
    {
        ...
        {
            var role = await roleManager.Roles.Include(p => p.Permissions).FirstOrDefaultAsync(p => p.Id == id);
            if (role == null)
            {
                return ResponseDto<RoleDto>.Fail("Role not found.", 404);
            }
        ...
        {
            var roles = await roleManager.Roles.Include(p => p.Permissions).ToListAsync();
