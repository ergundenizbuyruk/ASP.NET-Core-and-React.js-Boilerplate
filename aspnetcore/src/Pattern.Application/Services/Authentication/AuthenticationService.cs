...
namespace Pattern.Application.Services.Authentication
{
        ...
        : ApplicationService(unitOfWork, objectMapper), IAuthenticationService
    {
        public async Task<ResponseDto<AccessTokenDto>> CreateTokenAsync(LoginDto loginDto)
        {
            ...
            {
                return ResponseDto<AccessTokenDto>.Fail(
                    "Hesabınız kilitli durumdadır. Daha sonra giriş yapmayı deneyiniz.", 400);
            }
        ...
        {
            var existRefreshToken = await userRefreshTokenRepository.GetAll().Where(x => x.Code == refreshToken.Token)
                .SingleOrDefaultAsync();
